...
int oneseasonview() {
	...
	if (cJSON_IsArray(rankObj)) { //rankObj가 배열인지 확인
		...
		printf("이터널 리턴 %d 시즌 랭킹\n",season); //총 랭크 수 출력

		SetConsoleOutputCP(65001); //한글 출력을 위해 인코딩 변경
		...
		printf("\nAverage MMR of Top 1001 players: %.2f\n", totalmmr / 1001.0); //상위 1001명의 평균 MMR 출력
    }
